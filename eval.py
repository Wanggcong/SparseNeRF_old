...
def main(unused_argv):
  ...
  out_dir_gt = path.join(config.checkpoint_dir,'path_renders' if config.render_path else 'test_preds_gt')
  path_fn = lambda x: path.join(out_dir, x)
  path_fn_gt = lambda x: path.join(out_dir_gt, x)
  ...
  while True:
    ...
    try:
      # state = checkpoints.restore_checkpoint(config.checkpoint_dir, state, step=40000)
    ...
    for idx in range(dataset.size):
      ...
      if not config.render_path:
        ...
        if config.dataset_loader == 'dtu':
          ...
          psnr_mask.append(metric['psnr_masked'])
          ssim_mask.append(metric['ssim_masked'])
      ...
      if config.eval_save_output and (config.eval_render_interval > 0):
        if (idx % config.eval_render_interval) == 0:
          ...
          # save images for lpips evaluation
          if config.dataset_loader == 'dtu':
            utils.save_img_u8(rendering['rgb'], path_fn(f'color_{idx:03d}.png'))
            utils.save_img_u8(rgb_hat_fg, path_fn(f'color_{idx:03d}_lpips.png')) # render
            utils.save_img_u8(rgb_fg, path_fn_gt(f'color_{idx:03d}_lpips.png')) # gt
          else:
            utils.save_img_u8(rendering['rgb'], path_fn(f'color_{idx:03d}_lpips.png')) # render
            utils.save_img_u8(batch['rgb'], path_fn_gt(f'color_{idx:03d}_lpips.png')) # gt        

    print('###################################################')
    print('psnr:',psnr, sum(psnr)/len(psnr))
    print('ssim:',ssim, sum(ssim)/len(ssim))

    if config.dataset_loader == 'dtu':
      print('psnr_mask:',psnr_mask, sum(psnr_mask)/len(psnr_mask))
      print('ssim_mask:',ssim_mask, sum(ssim_mask)/len(ssim_mask))
    print('###################################################')    
